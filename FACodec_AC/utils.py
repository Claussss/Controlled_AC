...
def process_wav(filepath, fa_encoder, fa_decoder, out_dir, device):
    """
    Process one wav file using FACodec encoder/decoder and save the quantized latent codes.
    """
    try:
...
def process_files(file_list, fa_encoder, fa_decoder, out_dir, device, workers=4, sequential=False):
    ...
    if sequential:
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            futures = [executor.submit(process_wav, f, fa_encoder, fa_decoder, out_dir, device) for f in file_list]
